...
from store import COMMANDS,COMMAND_DESCRIPTIONS
...
class Phone(Field):
    ...
    def validate(self, number: str) -> bool:
        ...
        if number is None:
            return False
        phone_format = r'^\+\d{1,3}\d{9}$'  # Перевірка формату: +380XXXXXXXXX
...
class GuiUserView(UserView):
    def __init__(self):
        ...
        self.message_window.geometry(f"{screen_width//3}x{screen_height//10}+{(screen_width - screen_width//3) // 2}"
